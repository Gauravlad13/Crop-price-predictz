...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-8 relative"
      ...
      <div className="absolute inset-0 pointer-events-none" style={{background:'radial-gradient(ellipse 60% 50% at 20% 50%, rgba(74,124,89,0.06) 0%,transparent 60%)'}}/>
      <div className="w-full max-w-3xl bg-white rounded-5xl flex overflow-hidden relative z-10"
        style={{boxShadow:'0 40px 100px rgba(26,35,24,0.12)',minHeight:'560px'}}>
        <LeafPanel>
          ...
          <div className="relative z-10">
            <h2 className="font-display font-black text-4xl leading-none mb-4" style={{color:'var(--cream)',letterSpacing:'-1.5px'}}>
              Welcome<br/><em style={{color:'var(--marigold-light)'}}>Back</em>
            </h2>
            <p className="text-sm leading-relaxed" style={{color:'rgba(245,243,237,0.65)'}}>Access your crop price dashboard and stay ahead of the market every day.</p>
          </div>
          <p className="text-xs relative z-10" style={{color:'rgba(245,243,237,0.4)'}}>Trusted by 2,400+ farmers across India</p>
        </LeafPanel>

        <div className="flex-1 p-12 flex flex-col justify-center">
          <h2 className="font-display font-black text-3xl mb-1" style={{color:'var(--ink)',letterSpacing:'-0.5px'}}>Sign in to your account</h2>
          <p className="text-sm mb-8" style={{color:'var(--ink-soft)'}}>Enter your credentials to continue</p>
          ...
          {[['Email address','email','email','farmer@email.com','✉'],['Password','password','password','••••••••','🔒']].map(([label,name,type,ph,icon])=>(
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            {loading ? <span className="w-5 h-5 border-2 rounded-full animate-spin" style={{borderColor:'rgba(245,243,237,0.3)',borderTopColor:'var(--cream)'}}/> : <>Sign In <span>→</span></>}
          </button>

          <p className="text-sm text-center mt-6" style={{color:'var(--ink-soft)'}}>
            Don't have an account? <Link to="/register" className="font-semibold" style={{color:'var(--fern)'}}>Create one free</Link>
          </p>
